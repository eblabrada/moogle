...
Lo primero que el usuario debe hacer para poder usar este proyecto es \href{https://learn.microsoft.com/es-es/dotnet/core/install/}{instalar {\tt \color{gray45}.NET Core 6.0}}.
Luego, debe pararse en la carpeta del proyecto y dependiendo de tu sistema operativo hacer lo siguiente:

\begin{itemize}
	\item {\bf Linux:} Debe tener instalado {\tt \color{gray45} make}. Si no lo tiene instalado
...
Despu\'es de hacer lo anterior abra en su navegador \href{http://localhost:5000}{http://localhost:5000} y podr\'as usar {\bf Moogle!} introduciendo tu b\'usqueda en la ``entrada'' y luego presionando el bot\'on ``Buscar''.

\section*{Motor de b\'usqueda}

El motor de búsqueda usa un modelo vectorial que computa para una {\it query} dada 
...
{\it Cosine Similarity} para computar la relevancia. Para computar el vector {\it TF-IDF} se hace uso de la fórmula:
...
Después de hacer lo anterior necesitamos calcular la ``similitud'' entre el vector
{\it document} y el vector {\it query} para lo cual se hace uso de
{\it Cosine Similarity}.  La idea es intentar estimar el ``ángulo'' comprendido entre el vector
{\it query} y el vector {\it document}: mientras menor sea este ángulo, mayor 
``similitud'' tendrán estos vectores. Para lo anterior se hace uso de la f\'ormula:
...
Esta clase tiene los m\'etodos necesarios en el proyecto.
